...
## YFC integration contract

This skill is vendored/adapted from Emil Kowalski's `apple-design` skill:
https://github.com/emilkowalski/skills/blob/main/skills/apple-design/SKILL.md

Copyright (c) 2026 Emil Kowalski. Licensed under the MIT License; see `LICENSE.md` in this
skill directory.

Within Your Fitness Coach this is a **conditional reference skill**, not the visual authority.

- YFC `product-designer` owns product direction, brand identity and composition.
- YFC `motion-design-engineer` owns production motion decisions and implementation quality.
- Use this skill to improve Apple-like interaction physics, materials, depth, typography,
  direct manipulation and native-feel behavior only when the task actually has that trigger.
- Preserve YFC sport-tech character and the lime/black/white brand core; do not turn the product
  into an iOS clone.
- In ordinary evolve tasks, do not use this skill as permission for a hidden global redesign.
- Accessibility, performance, product truth and the current task remain higher-priority constraints.
- When this skill is loaded as part of an active YFC task, that task counts as a specific question:
  **skip the upstream standalone Initial Response handshake and execute the task normally.**
