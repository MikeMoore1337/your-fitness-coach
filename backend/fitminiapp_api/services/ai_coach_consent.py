...
AI_COACH_PERSONAL_RETENTION_NOTICE = (
    "История диалога сохраняется отдельно в вашем аккаунте и удаляется вместе с разговором; "
    "она не превращается в долговременную память. Провайдер получает только ограниченный "
    "контекст текущего запроса по действующей политике сервиса."
)
