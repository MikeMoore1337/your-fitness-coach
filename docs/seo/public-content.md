...
| `/programs/full-body-3-days` | Самостоятельные пользователи: увидеть реальную программу на три дня | Один канонический read-only шаблон с упражнениями, подходами, повторами и отдыхом |
...
Публичная программа `/programs/full-body-3-days` устроена так же: манифест владеет маршрутом и
редакционным текстом, а read-only API `/api/v1/public/programs/full-body-3-days` собирает
обезличенную структуру из канонического seed-шаблона `strength-fullbody-3d` и общего каталога
упражнений. В ответе нет пользователя, назначения, сохранения или персонального веса.
...
- Страницы продукта и каноническая публичная программа используют `WebPage`, каталог знаний — `CollectionPage`.
...
публикуйте индексируемые черновики или искусственные скриншоты-заглушки. Публичный шаблон на три
дня остаётся информационным: события `public_program_opened` и `public_program_saved` не
используются, пока отдельный product flow не появится и не будет подтверждён.
