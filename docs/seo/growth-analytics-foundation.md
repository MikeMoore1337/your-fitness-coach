...
- product events идут через typed provider. Growth events отправляются только как `reachGoal` с
  exact goal id; обычные события передают только `yfc_event` и `yfc_surface`, кроме перечисленных
  ниже безопасных UX-размеров;
...
### UX Stage 6: evidence для основной навигации

Adapter передаёт только явно сопоставленные размеры из существующих закрытых enum:

- `section_navigation_selected` — `yfc_from_section`, `yfc_to_section`;
- `next_action_shown`, `next_action_clicked`, `next_action_completed` — `yfc_action_kind`,
  `yfc_position`.

Произвольные свойства события отбрасываются. Эти события не содержат user ID, содержимого
тренировки, значений тела или пользовательского текста.

Stage 1 отправлял `from_section`/`to_section` локально, но прежний production adapter их отбрасывал.
Историческое распределение переходов по назначениям через этот путь недоступно. Текущий вердикт:
`INSUFFICIENT_EVIDENCE -> KEEP NAV`; «План» остаётся на прежнем месте.

Новая выборка начинается с первой production-версии с исправленным adapter. Минимальное окно —
28 полных дней, чтобы охватить четыре недельных цикла; при неполных или редких данных окно следует
продлить. Отчёт фиксирует даты и версию, число визитов/пользователей и событий, доступный контекст
популяции и трафика, а `mobile_web` и `tma` показывает отдельно от `desktop_web`.

Сравниваются прямые переходы по `to_section`, показы и выборы Next Action по
`action_kind`/`position`, а также контекстные входы Today в программу или тренировку и доступные
сигналы использования разделов из «Ещё». Текущее событие выбора раздела не отмечает открытие
панели «Ещё» и не содержит источник-позицию: его выборы не дают знаменатель для оценки
обнаруживаемости. Отчёт должен явно сохранить это ограничение и не выводить частоту использования
из UI-кода, фикстур, личного поведения или недоступной исторической выборки. Пороговые проценты и
персональная/динамическая перестановка навигации не вводятся.
