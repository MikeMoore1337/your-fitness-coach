# AI Coach: generic-only foundation

Task 88 добавляет безопасный backend-контур для первой beta-версии AI Coach. Он не
заменяет детерминированные расчёты, не получает доступ к аккаунту пользователя и не
включает публичный chat-интерфейс.
...
`AI_COACH_DATA_POLICY=verified_generic_only`. Для Task 88 production route остаётся
выключенным; текущая конфигурация является foundation и controlled-unavailable path.
...
версии, provider/model metadata, outcome, latency, attempts, nullable usage и safe error
code.
