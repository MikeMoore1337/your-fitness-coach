...
def test_specialized_qa_skills_are_manifested_and_present() -> None:
    ...
    assert QA_SKILLS <= set(manifest["skills"])
...
def test_qa_router_and_routing_guide_reference_specialists() -> None:
    ...
    architecture = (
        ROOT / ".agents/references/QA_AUTOMATION_ARCHITECTURE.md"
    ).read_text(encoding="utf-8")

    for skill in QA_SKILLS:
        assert f"$" + skill in qa or skill in qa
        assert f"$" + skill in routing or skill in routing
...
def test_ui_audit_remains_separate_from_playwright_testing() -> None:
    playwright = (
        ROOT / ".agents/skills/playwright-testing/SKILL.md"
    ).read_text(encoding="utf-8")
